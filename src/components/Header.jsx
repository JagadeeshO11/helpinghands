...
import { Menu, X, ChevronDown, Zap, Phone, Heart, LogIn } from "lucide-react"
...
const NAV_LINKS = [
  ...
  { label: "About Us", to: "/about", dropdown: true, menu: [
    ...
    ]}, { label: "Legal & Terms", to: "/about/legal-terms" }
  ]},
...
function Dropdown({ link }) {
  ...
  return <div className="relative" onMouseEnter={() => setOpen(true)} onMouseLeave={() => { setOpen(false); setNestedOpen(false) }}>
    ...
    <div className={`absolute left-1/2 top-full z-[120] mt-3 w-64 -translate-x-1/2 rounded-2xl border border-border bg-card p-2 shadow-xl transition-all duration-200 ${open ? "visible translate-y-0 opacity-100" : "invisible -translate-y-1 opacity-0"}`}>
      {link.menu.map(item => item.nested ? <div key={item.to} className="relative" onMouseEnter={() => setNestedOpen(true)}>
        ...
        {nestedOpen && <div className="absolute left-full top-0 ml-2 w-52 rounded-2xl border border-border bg-card p-2 shadow-xl">{item.children.map(c => <NavLink key={c.to} to={c.to} className="block rounded-xl px-3 py-2.5 text-sm font-semibold text-primary hover:bg-primary-soft">{c.label}</NavLink>)}</div>}
...
function LoginDropdown({ onNavigate }) {
  ...
  return <div className="relative shrink-0" onMouseLeave={() => setOpen(false)}>
    ...
    <div className={`absolute right-0 top-full z-[150] mt-2 w-48 rounded-2xl border border-border bg-white p-2 shadow-xl transition-all ${open ? "visible opacity-100" : "invisible -translate-y-1 opacity-0"}`} role="menu">
  ...
  return <>
    ...
    <header className={`sticky top-0 z-[100] w-full max-w-full border-b border-border bg-card/95 backdrop-blur-md transition-shadow ${scrolled ? "shadow-md" : ""}`}>
      <div className="flex h-[68px] w-full min-w-0 items-center justify-between gap-1 px-2 sm:h-[82px] sm:gap-2 sm:px-5 lg:h-[97px] lg:px-6">
        ...
        <NavLink to="/" className="group flex min-w-0 max-w-[calc(100%-175px)] shrink items-center gap-1 sm:max-w-none sm:gap-3"><span className="grid size-12 shrink-0 place-items-center sm:size-18 lg:size-24"><img src={LOGO_URL} alt="Helping Hands Foundation" className="size-10 object-contain sm:size-16 lg:size-21" /></span><span className="min-w-0 leading-none"><span className="block truncate font-heading text-[9px] font-extrabold text-primary sm:text-[14px] lg:text-[16px]">HELPING HANDS</span><span className="mt-1 block truncate text-[5px] font-semibold uppercase tracking-[0.08em] text-muted-foreground sm:text-[9px] lg:text-[10px]">Foundation</span></span></NavLink>
