  ...
  return (
    <section id="programs" className="w-full px-4 py-12 sm:px-6 sm:py-16 lg:px-8 lg:py-20">
      <div className="mx-auto w-full max-w-[1320px]">
        ...
        <div className="mt-6 flex gap-4 overflow-x-auto pb-3 snap-x snap-mandatory no-scrollbar sm:mt-8 sm:grid sm:grid-cols-2 sm:gap-5 lg:grid-cols-3 lg:gap-6">
          {programs.map((program, i) => (
            <FadeIn key={program.id} delay={i * 0.1} className="min-w-[84%] snap-start sm:min-w-0">
              <article className="group h-full overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg sm:rounded-3xl">
                <div className="relative aspect-[4/3] overflow-hidden">
                  ...
                  />
                  {program.tag && (
                    <span className="absolute left-3 top-3 rounded-full bg-teal px-2.5 py-1 text-[8px] font-bold text-white sm:text-[9px]">
                      {program.tag}
                    </span>
                  )}
                </div>
                <div className="p-4 sm:p-5">
                  ...
                  <NavLink to="/programs" className="mt-4 inline-flex min-h-9 items-center gap-1.5 text-xs font-bold text-teal transition hover:gap-2.5">
