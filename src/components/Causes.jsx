...
export default function Causes() {
  return (
    <section id="causes" className="w-full px-4 py-12 sm:px-6 sm:py-16 lg:px-8 lg:py-20">
          ...
          {causes.map((cause, i) => (
            <FadeIn key={cause.id} delay={i * 0.08}>
              <article className="group flex h-full min-h-[250px] flex-col rounded-2xl border border-[#eee9df] bg-[#fffdfa] p-5 shadow-[0_6px_20px_rgba(40,63,70,0.05)] transition duration-300 hover:-translate-y-1 hover:shadow-lg sm:rounded-3xl sm:p-6">
                ...
                <NavLink to="/programs" className={`mt-auto pt-5 inline-flex min-h-9 items-center gap-1.5 text-xs font-bold transition hover:gap-2.5 ${cause.color === "accent" ? "text-accent" : "text-teal"}`}>
