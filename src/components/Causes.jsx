      ...
      <div className="mt-4 grid grid-cols-1 gap-3 sm:mt-6 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">
        {causes.map((cause, i) => (
          <FadeIn key={cause.id} delay={i * 0.08}>
            <article className="group h-full rounded-xl border border-[#eee9df] bg-[#fffdfa] p-3 shadow-[0_4px_14px_rgba(40,63,70,0.05)] transition duration-300 hover:-translate-y-1 hover:shadow-lg sm:rounded-2xl sm:p-5">
              ...
              <h3 className="mt-2 text-[13px] font-bold text-primary sm:mt-4 sm:text-[15px]">{cause.title}</h3>
              <ul className="mt-1.5 space-y-0.5 text-[10px] leading-[1.45] text-muted-foreground sm:mt-3 sm:space-y-1 sm:text-[11px]">
                {cause.items.map((item) => <li key={item}>• {item}</li>)}
              </ul>
              <NavLink
                to="/programs"
                className={`mt-2 inline-flex min-h-7 items-center gap-1 text-[10px] font-bold transition hover:gap-2 sm:mt-4 sm:text-[10px] ${cause.color === "accent" ? "text-accent" : "text-teal"}`}
              >
