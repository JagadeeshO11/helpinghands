  ...
  return (
    <div className="border-b border-white/15">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        className="flex min-h-11 w-full items-center justify-between py-3 text-left text-[8px] font-bold uppercase tracking-[0.12em] text-white sm:text-sm"
      >
        {title}
  ...
  return (
    <footer id="footer" className="relative mt-2 bg-teal-dark text-white">
        ...
        <div className="lg:grid lg:grid-cols-4 lg:gap-10">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="grid size-9 place-items-center rounded-full border border-white/20 sm:size-11">
                <Logo variant="light" className="size-5 sm:size-6" />
              </span>
              <span>
            ...
            <div className="mt-3 flex gap-1.5 sm:mt-5 sm:gap-2.5">
              {SOCIALS.map(({ icon: Icon, label }) => (
                <a
                  ...
                  href="#"
                  aria-label={label}
                  className="grid size-7 place-items-center rounded-full bg-white/10 text-white transition hover:bg-accent sm:size-9"
                >
          ...
          <div className="mt-4 lg:hidden">
            <AccordionSection title="Quick Links">
              <ul className="space-y-1.5 pb-1">
                {QUICK_LINKS.map((link) => (
                  <li key={link.to}>
                    <NavLink to={link.to} className="text-[8px] text-white/70 transition hover:text-accent sm:text-sm">
                      {link.label}
                    </NavLink>
                  </li>
                ))}
            ...
            <AccordionSection title="Our Services">
              <ul className="space-y-1.5 pb-1">
                {SERVICES.map((s) => (
                  <li key={s} className="text-[8px] text-white/70 sm:text-sm">{s}</li>
                ))}
              </ul>
            </AccordionSection>
            <AccordionSection title="Contact Us">
              <ul className="space-y-2 pb-1">
                {CONTACT_ITEMS.map(({ icon: Icon, text }) => (
                  <li key={text} className="flex items-start gap-2 text-[8px] text-white/70 sm:text-sm">
          ...
          <div className="hidden lg:col-span-3 lg:grid lg:grid-cols-3 lg:gap-8">
            <div>
              <h3 className="text-[11px] font-bold uppercase tracking-[0.12em]">Quick Links</h3>
              <ul className="mt-5 space-y-3">
                {QUICK_LINKS.map((link) => (
                  <li key={link.to}>
                    <NavLink to={link.to} className="text-[11px] text-white/70 transition hover:text-accent">
                      {link.label}
                    </NavLink>
                  </li>
                ))}
            ...
            <div>
              <h3 className="text-[11px] font-bold uppercase tracking-[0.12em]">Our Services</h3>
              <ul className="mt-5 space-y-3 text-[11px] text-white/70">
                {SERVICES.map((s) => <li key={s}>{s}</li>)}
              </ul>
            </div>
            <div>
              <h3 className="text-[11px] font-bold uppercase tracking-[0.12em]">Contact Us</h3>
              <ul className="mt-5 space-y-3">
                {CONTACT_ITEMS.map(({ icon: Icon, text }) => (
                  <li key={text} className="flex items-start gap-2.5 text-[11px] text-white/70">
      ...
      <button
        ...
        className="absolute -top-3 right-3 grid size-8 place-items-center rounded-full bg-white text-teal shadow-md transition hover:bg-accent hover:text-white sm:-top-5 sm:right-4 sm:size-11"
      >
