...
export default function AdminLayout() {
  ...
  const linkClass = ({ isActive }) =>
    `flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-[11px] font-semibold transition sm:text-sm ${
      isActive ? "bg-teal text-white shadow-sm" : "text-primary hover:bg-primary-soft"
    }`
  ...
  return (
    <div className="flex min-h-screen bg-muted/30">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-56 flex-col border-r border-border bg-card transition-transform duration-300 lg:static lg:translate-x-0 sm:w-64 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex h-14 items-center gap-2 border-b border-border px-4 sm:h-16">
          <span className="grid size-7 place-items-center rounded-full border border-[#8fbe78] bg-[#eef7e9] sm:size-8">
        ...
        <div className="border-t border-border p-3">
          <button onClick={handleLogout} className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-[11px] font-semibold text-muted-foreground transition hover:bg-orange-50 hover:text-accent sm:text-sm">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-20 flex h-14 items-center gap-3 border-b border-border bg-card/95 px-3 backdrop-blur-md sm:h-16 sm:px-6">
          <button onClick={() => setSidebarOpen(true)} className="grid size-8 place-items-center rounded-xl text-teal hover:bg-primary-soft lg:hidden">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <span className="rounded-full bg-green-50 px-2.5 py-1 text-[8px] font-bold text-green-700 sm:text-[10px]">Super Admin</span>
          </div>
        </header>
